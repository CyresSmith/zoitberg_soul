## [2026-04-15] Multi-Project Policy | Strict Isolation in Knowledge Base
- **Що зроблено:** Встановлено чітке правило багатопроєктності. Газтехпрайм зафіксовано як перший, але не єдиний проєкт. Всі знання, код і контексти майбутніх проєктів мають бути суворо ізольованими і не змішуватися.
- **Файли:** `SOUL.md`, `index.md`, `KNOWLEDGE.md`.
- **Конвенції:** Перед початком роботи (або після перезапуску) агент завжди перевіряє змінну `Active Project` в `index.md`. Кожен проєкт має власну підпапку та свою Obsidian-нотатку (наприклад, `[[Назва Проєкту]]`).
- **Git Hash:** N/A (Wiki update only)
